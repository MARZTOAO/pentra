          ...
          {incoming.map((row) => (
            <Row key={row.friendship_id} row={row}>
              <button
                ...
                className="rounded-lg bg-accent px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-accent-hi disabled:opacity-50"
              >
                Accept
              </button>
              <button
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-line bg-surface p-3">
      <Link to={`/u/${row.username}`} className="relative shrink-0">
        <div className="h-11 w-11 overflow-hidden rounded-full border border-line bg-surface-2">
          {row.avatar_url ? (
            <img src={row.avatar_url} alt="" className="h-full w-full object-cover" />
          ) : (
            <div className="flex h-full w-full items-center justify-center text-base font-bold text-muted">
